...
#ifndef UI_FONT_HANYIWENHEI24
#define UI_FONT_HANYIWENHEI24 1
...
static uint8_t __g_font_buf[406]; //如bin文件存在SPI FLASH可使用此buff
static lv_fs_file_t font_f;       //文件操作指针
static lv_fs_res_t font_res;      //文件系统返回值变量
static uint32_t font_resCount;    //字节数变量
static uint8_t isFontOpened = 0;
static uint8_t *__user_font_getdata(int offset, int size){
    //如字模保存在SPI FLASH, SPIFLASH_Read(__g_font_buf,offset,size);
    //如字模已加载到SDRAM,直接返回偏移地址即可如:return (uint8_t*)(sdram_fontddr+offset);
    if(!isFontOpened) {
        font_res = lv_fs_open(&font_f, "S:/The Vision L/fonts/ui_font_HanyiWenhei24.bin", LV_FS_MODE_RD);   //打开字库
        isFontOpened = 1;
    }    
...
//字模高度：29
//XBF字体,外部bin文件
const lv_font_t ui_font_HanyiWenhei24 = {
